...
// TODO: renew source
public class SpreadsheetSimpleTableTranslator extends SimpleElementReplacement {
	...
	public SpreadsheetSimpleTableTranslator(DocumentStyle style,
	...
	private void translateRows(LWXMLPushbackReader in, LWXMLWriter out)
			throws IOException {
		for (int i = 0; i < currentTableSize.getRows(); i++) {
			LWXMLEvent event = in.readEvent();
	...
	private void translateCells(LWXMLPushbackReader in, LWXMLWriter out)
			throws IOException {
		for (int i = 0; i < currentTableSize.getColumns(); i++) {
			LWXMLEvent event = in.readEvent();
